...
namespace SDLab_GUI.UIComponents.Editors
{
    ...
    public partial class MIDIInterfaceEditor : ContentPage
    {
        ...
        private Command modalBoxCloseEvent;
        ...
        /// </summary>
        internal Action ModalBoxCloseEvent
        {
            set
            {
                modalBoxCloseEvent = new Command(() => { value(); });
                modalBoxCloseBTN.Command = modalBoxCloseEvent;
            }
        }

        internal PianoRollGraphicsView PianoRoll { get => pianoRoll; }
        ...
        private void addMIDITrackTemplateComponentUI(TrackItem newTemplateAP)
        {
            if (newTemplateAP != null)
            {
                ...
                mainPlayBTN.Clicked += async delegate {
                    ...
                    switch (mainPlayBTN.Source.ToString().Split(" ")[1])
                    {
                        case "pause_button.png":
                            ...
                            PianoRoll.stopPianoTimeLineMovement();
                            break;

                        case "play_solid_full.png":
                            mainPlayBTN.Source = "pause_button.png";
                            await Task.Run(() => {
                                float timeLimit = ((MIDITrackProvider)MIDIAudioTrack.TrackAudioProvider).renderMIDIWaveform(newTemplateAP.TrackAudioProvider);
                                ((MIDITrackProvider)MIDIAudioTrack.TrackAudioProvider).PlayMIDI();
                                PianoRoll.startPianoTimeLineMovement(timeLimit);
                            });
                            break;
                    }
                ...
                midiWaveformRenderBTN.Command = new Command(async () => {
                    mainPageRefObj.ShowLoadingModalSplashScreen();
                    ...
                    List<Page> topNavigationStack = new List<Page>();

                    while (Navigation.ModalStack[Navigation.ModalStack.Count - 1].AutomationId != "LoadingScreenModal")
                    {
                        topNavigationStack.Add(Navigation.ModalStack[Navigation.ModalStack.Count - 1]);
                        await Navigation.PopModalAsync();
                    }

                    mainPageRefObj.HideLoadingModalSplashScreen();

                    for(int i = 0; i < topNavigationStack.Count; i++)
                    {
                        await Navigation.PushModalAsync(topNavigationStack[topNavigationStack.Count - 1 - i]);
                    }
                });
            }
        }
    ...
    internal class PianoRollGraphicsView : GraphicsView
    {
        ...
        IDispatcherTimer timelineMovementTimer;

        private Action endNotesHighlightTutorialTaskAction;
        ...
        public IDispatcherTimer TimelineMovementTimer
        {
            get => timelineMovementTimer;
            set => timelineMovementTimer = value;
        }

        private Global.struct_coordinates lastHoveredCoordinate = new Global.struct_coordinates() { x = -1, y = -1 };
        public Action EndNotesHighlightTutorialTaskAction { get => endNotesHighlightTutorialTaskAction; set => endNotesHighlightTutorialTaskAction = value; }
        ...
        public PianoRollGraphicsView(StackLayout parent, struct_timelineInfo timeLineInfo, struct_keyboardInfo keyboardInfo, JuceAudioProvider MIDIAP)
        {
            ...
            TimelineMovementTimer = Dispatcher.CreateTimer();

            TimelineMovementTimer.Interval = TimeSpan.FromMilliseconds(33);

            TimelineMovementTimer.Tick += delegate {
                if(drawable.CurrentTime < TimeLimit) drawable.CurrentTime += 33f / 100f;
                else drawable.CurrentTime = 0;
        ...
        public void toggleNote(float x, float y)
        {
            int timeIndex = (int)(x / drawable.TI.timeUnitSquareWidth);
            ...
            if (lastHoveredCoordinate.x == x && lastHoveredCoordinate.y == y) return;
            ...
            lastHoveredCoordinate = targetNoteCoordinates;

            drawable.ToggleNote(targetNoteCoordinates);

            int countHighlightedActiveNotes = 0;

            for(int k = 0; k < drawable.HighlightedNotes.Count; k++)
            {
                if (((MIDITrackProvider)drawable.MIDIAP).ActiveNotesLazyList.Contains(drawable.HighlightedNotes[k]))
                {
                    countHighlightedActiveNotes++;
                }
            }

            if(countHighlightedActiveNotes > 0 && countHighlightedActiveNotes == drawable.HighlightedNotes.Count)
            {
                EndNotesHighlightTutorialTaskAction();
            }

            Invalidate();
        }

        public void highlightNote(Global.struct_coordinates noteCoords)
        {
            drawable.HighLightNote(noteCoords);
            Invalidate();
        }

        public void startPianoTimeLineMovement(float timeLimit)
        {
            TimeLimit = timeLimit;
            drawable.CurrentTime = 0;
            TimelineMovementTimer.Start();
        }

        public void stopPianoTimeLineMovement()
        {
            TimelineMovementTimer.Stop();
        }
    }

    internal class PianoRollDrawable : IDrawable
    {
        ...
        private List<Global.struct_coordinates> highlightedNotes = new List<Global.struct_coordinates>();

        public struct_keyboardInfo KI { get => _KI; set => _KI = value; }
        public struct_timelineInfo TI { get => _TI; set => _TI = value; }
        public List<Global.struct_coordinates> HighlightedNotes { get => highlightedNotes; set => highlightedNotes = value; }
        ...
        public void Draw(ICanvas canvas, RectF dirtyRect)
        {
            ...
            canvas.StrokeColor = Color.FromArgb("#bc4338");
            canvas.DrawLine(currentTime * TI.timeUnitSquareWidth, 0, currentTime * TI.timeUnitSquareWidth, (KI.numOctaves * KI.numWhiteKeysPerOctave + 1) * 22);

            for (int i = 0; i < highlightedNotes.Count; i++)
            {
                (float a, int b) = calcHeightAndNoteFromIndex(highlightedNotes[i]);
                float rectY = a;
                float rectX = highlightedNotes[i].x * TI.timeUnitSquareWidth;

                canvas.FillColor = Color.FromArgb("#99ffffff");
        ...
        public void HighLightNote(Global.struct_coordinates noteCoords)
        {
            if (!highlightedNotes.Contains(noteCoords))
            {
                highlightedNotes.Add(noteCoords);
            }
        }
